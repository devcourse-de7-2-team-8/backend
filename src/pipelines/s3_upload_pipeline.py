...
def convert_session_data_to_csv(years: List[str]):
    ...
    file_paths = [
        get_path(f"data/sessions/year={year}/ev_charging_session.xlsx") for
        year in years]
...
def convert_station_data_to_csv(station_files: List[str]):
    ...
    file_paths = [get_path(f"data/stations/{stations}.xlsx")
                  for stations in station_files]
...
convert_session_task = Task("convert_session_task",
                            lambda: convert_session_data_to_csv(
                                ["2024", "2025"]))
convert_station_task = Task("convert_station_task",
                            lambda: convert_station_data_to_csv(
                                ['seoul_station']))

upload_to_s3_task = Task("upload_to_s3_task",
                         lambda: upload_to_s3(convert_session_task.result
                                              + convert_station_task.result))
