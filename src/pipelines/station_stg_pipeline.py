...
def create_station_stg():
    ...
    try:
        ...
        # --------------------------------------------------------------
        cur.execute(f"""
        CREATE TABLE IF NOT EXISTS {DB}.{SCHEMA}.station_stg (
        ...
        # 1-2. TEMP 테이블 생성
        cur.execute(f"""
        CREATE OR REPLACE TEMP TABLE {DB}.{SCHEMA}.tmp_station_stg LIKE {DB}.{SCHEMA}.station_stg;
        """)

        # S3 데이터를 TEMP 테이블로 COPY
        cur.execute(f"""
        COPY INTO {DB}.{SCHEMA}.tmp_station_stg
        FROM 's3://{s3_bucket}/data/stations/seoul_station.csv'
        CREDENTIALS=(AWS_KEY_ID='{aws_key}' AWS_SECRET_KEY='{aws_secret}')
        FILE_FORMAT=(TYPE=CSV FIELD_OPTIONALLY_ENCLOSED_BY='"' SKIP_HEADER=1);
        """)
        ...
        cur.execute(f"""
        MERGE INTO {DB}.{SCHEMA}.station_stg t
        USING {DB}.{SCHEMA}.tmp_station_stg s
        ...
        print(f"{DB}.{SCHEMA}.station_stg 테이블 생성 및 S3 데이터 로드 완료!")
...
# Task 래핑 그대로 보존
t = Task("station_stg 생성", create_station_stg)
